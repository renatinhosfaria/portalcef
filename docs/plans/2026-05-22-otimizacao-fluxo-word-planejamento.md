...
**Decisao de escopo da execucao:**

Esta entrega corrige apenas o fluxo de documentos Word de `plano-aula`.
O modulo `prova` possui fluxo espelhado e ainda chama `PdfGeneratorService.gerarParaImpressao`
de forma sincrona em `ProvaService.aprovarDocumento`, mas sera tratado em uma entrega separada.

Motivos:

- a investigacao de lentidao foi feita sobre o fluxo de revisao de planejamento usado por Patricia Aline;
- `prova` exige campos de status de PDF proprios em `provaDocumento`, fila/worker dedicados ou generalizacao segura da fila atual;
- a UI de provas em `apps/planejamento/features/prova/*` precisa de estados e reprocessamento equivalentes aos de `plano-aula`;
- misturar os dois fluxos no mesmo lote aumentaria risco de regressao em impressao de provas.

Escopo recomendado para a proxima entrega:

- adicionar status de PDF em `provaDocumento`;
- aprovar documento de prova sem aguardar conversao Word -> PDF;
- enfileirar geracao de PDF para prova;
- reutilizar item ativo do SharePoint e sincronizar Word antes de converter;
- exibir status/reprocessamento na UI de provas;
- registrar observabilidade `pdf_impressao` tambem com `arquivo.provaId`.
...
- **Provas com mesmo problema:** fora do escopo desta entrega; criar plano separado antes de considerar a otimizacao ampla para todos os documentos Word do planejamento.
...
- Escopo documentado: os criterios acima se aplicam ao fluxo de `plano-aula`; o modulo `prova` permanece como trabalho posterior.
