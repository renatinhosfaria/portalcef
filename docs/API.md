...
- Rejeita produto não marcado como pré-venda com erro `PRODUCT_NOT_PRE_SALE`.
...
**Roles base:** `master`, `diretora_geral`, `gerente_unidade`, `gerente_financeiro`, `auxiliar_administrativo`.
No `loja-admin`, `auxiliar_administrativo` possui a mesma superfície de permissões de `gerente_unidade`; `gerente_financeiro` mantém apenas as operações financeiras e de atendimento liberadas nos controllers.
